#!/usr/bin/env python3
"""
Adds `agentlens explore` - the sandbox, clickable.

Run from inside agentlens-project/ (after add_sandbox.py):

    python add_explorer.py
    ruff check --fix . && ruff check . && mypy agentlens && pytest -q
    python -m agentlens.cli explore && open examples/explorer.html

What it is
----------
One self-contained HTML file. Pick a table, pick a change, see what breaks -
and see what a lineage-only tool would have told you instead. No server, no
build step, no network. Open it from disk, mail it to someone, put it in the
repo.

The page is a *viewer*. It carries the forked graph, the skill text, and the
agent-to-skill map that `sandbox` computes, so the browser can answer
"what if" instantly for any column you type. The CLI remains authoritative:
`sandbox.py` and the ~40 lines of JS that mirror it are marked in both places
...
# ===========================================================================
FILES["agentlens/explorer.py"] = '''"""Build a self-contained, clickable view of the sandbox.
...
# ===========================================================================
FILES["tests/test_explorer.py"] = '''"""The explorer payload. No browser needed - we assert on what gets embedded."""
...
PARSER = '''    p = sub.add_parser("explore", help="build a clickable HTML view of the sandbox")
...
def patch_cli() -> bool:
    ...
    if "cmd_explore" in cli:
        print("  ok     cli: explore (already applied)")
        return True

    if "\nimport os\n" not in cli:
        cli = cli.replace("import json\n", "import json\nimport os\n", 1)

    imp = "from .emitter import Emitter"
    if imp not in cli:
        print("  MISS   cli: emitter import line")
        return False
    cli = cli.replace(imp, imp + "\nfrom .explorer import write as explorer_write", 1)
    ...
    with open(path, "w", encoding="utf-8") as fh:
        fh.write(cli)
    print("  patch  cli: explore command")
    return True
...
def main() -> int:
    if not os.path.isdir("agentlens"):
        ...
        return 1
    if not os.path.exists("agentlens/sandbox.py"):
        print("Run add_sandbox.py first - the explorer is a view over it.")
        return 1
  ...
  Done.
      ...
      python -m agentlens.cli explore
      open examples/explorer.html

  Click a table, click a column. The number that matters is in the blue bar:
  how many a lineage-only tool would have flagged, versus how many actually
  break. On the demo repo, `line_total` is 7 versus 3.

  It is one file with no external requests, so it commits to the repo and
  opens from disk. Re-run `explore` after changing your skills.
""")
    return 0
