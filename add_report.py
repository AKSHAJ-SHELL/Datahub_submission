#!/usr/bin/env python3
"""
Adds HTML report rendering to AgentLens.

Run from inside agentlens-project/:

    python add_report.py

Writes agentlens/report.py and patches cli.py to accept --html on
`impact` and `guard`.
"""
...
REPORT_PY = r'''"""Render a blast radius report as a self-contained HTML page.
...
def main() -> int:
    ...
    with open("agentlens/report.py", "w", encoding="utf-8") as fh:
        fh.write(REPORT_PY)
    print("  write  agentlens/report.py")

    # --- patch cli.py -----------------------------------------------------
    with open("agentlens/cli.py", encoding="utf-8") as fh:
        cli = fh.read()

    if "from .report import" not in cli:
        cli = cli.replace(
...
def cmd_guard""",
    ...
    # guard: write html with the action log
    cli = cli.replace(
        """        print(f"\\n  wrote {args.json}")""",
        """        print(f"\\n  wrote {args.json}")
    if args.html:
    ...
    with open("agentlens/cli.py", "w", encoding="utf-8") as fh:
        fh.write(cli)
    print("  patch  agentlens/cli.py")

    ok = all(s in cli for s in ("--html", "write_html", "from .report import"))
    print(f"\n  patch verified: {ok}")
    if not ok:
        print("  (if False, tell Claude - the cli.py text didn't match)")
